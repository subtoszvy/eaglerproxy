...
export class Player extends EventEmitter {
    ...
    initListeners() {
        ...
        this.ws.on("message", (msg) => {
            ...
            if (decoder && decoder.sentAfterHandshake) {
                ...
                try {
                    const parsed = this.deserializer.parsePacketBuffer(msg)?.data, translated = this.translator.translatePacketClient(parsed.params, parsed.name), packetData = {
                        name: translated[0],
                        params: translated[1],
                        cancel: false,
                    };
                    ...
                    if (!packetData.cancel) {
                        this._sendPacketToServer(msg);
                    }
                }
                catch (err) {
                    this._logger.debug(`Client ${this
    ...
    switchServers(options) {
        ...
        return new Promise(async (res, rej) => {
            ...
            this.ws.send(this.serializer.createPacketBuffer({
                name: "chat",
                params: {
                ...
                if (!stream) {
                    if (switchingServers) {
                        if (meta.name == "login" && meta.state == states.PLAY && uuid) {
                            ...
                            const pckSeq = BungeeUtil.getRespawnSequence(packet, this.serializer);
                            this.ws.send(this.serializer.createPacketBuffer({
                                name: "login",
                                params: packet,
                            }));
                    ...
                    const translated = this.translator.translatePacketServer(packet, meta.name), eventData = {
                        name: translated[0],
                        params: translated[1],
                        cancel: false,
                    };
