...
export class PluginManager extends EventEmitter {
            ...
            if (details.isFile()) {
                ...
                try {
                    ...
                    for (const requirement of metadata.requirements) {
                        ...
                        if (semver.validRange(requirement.version) == null &&
        ...
        for (const [id, plugin] of plugins) {
            for (const req of plugin.requirements) {
                ...
                if (req.id == "eaglerproxy") {
                    if (!semver.satisfies(PROXY_VERSION, req.version) &&
                        req.version != "any") {
                    ...
                    if (!semver.satisfies(dep.version, req.version) &&
                        req.version != "any") {
    ...
    async _loadPlugins(plugins, order) {
        ...
        for (const id of order) {
            ...
            try {
                const imp = await import(process.platform == "win32"
                    ? pathToFileURL(pathUtil.join(pluginMeta.path, pluginMeta.entry_point)).toString()
