...
export class Proxy extends EventEmitter {
    ...
    async init() {
        ...
        if (this.config.tls && this.config.tls.enabled) {
            ...
            this.httpServer = http
                .createServer((req, res) => this._handleNonWSRequest(req, res, this.config))
                .listen(this.config.bindPort || 8080, this.config.bindHost || "127.0.0.1");
            this.wsServer = new WebSocketServer({
                noServer: true,
            });
    ...
    async _handleWSConnection(ws) {
        ...
        setTimeout(() => {
            if (!handled) {
                this.initalHandlerLogger.warn(`Disconnecting client ${player
                    ? player.username ??
                        `[/${ws._socket.remoteAddress}:${ws._socket.remotePort}`
                    : `[/${ws._socket.remoteAddress}:${ws._socket.remotePort}`} due to connection timing out.`);
                if (player)
        ...
        try {
            if (firstPacket.toString() === "Accept: MOTD") {
                ...
                if (loginPacket.gameVersion != VANILLA_PROTOCOL_VERSION) {
                    ...
                    player.disconnect(`${Enums.ChatColor.RED}Your EaglercraftX version is too ${loginPacket.networkVersion > NETWORK_VERSION ? "new" : "old"}! Please ${loginPacket.networkVersion > NETWORK_VERSION
                        ? "downgrade"
                        : "update"}.`);
                    return;
                }
                ...
                else if (this.players.get(player.username) != null ||
                ...
                const prom = await Promise.all([
                    ...
                    (await player.read(Enums.PacketId.CSSetSkinPacket)),
                ]), skin = prom[1], obj = new EaglerSkins.EaglerSkin();
                obj.owner = player;
    ...
    async _handleWSConnectionReq(req, socket, head) {
        ...
        if (this.config.origins.originBlacklist != null &&
            this.config.origins.originBlacklist.some((host) => Util.areDomainsEqual(host, origin))) {
            socket.destroy();
            return;
        }
        if (this.config.origins.originWhitelist != null &&
            !this.config.origins.originWhitelist.some((host) => Util.areDomainsEqual(host, origin))) {
            socket.destroy();
            return;
        }
        try {
